...
void draw() {
  ...
  int new_x = int (map(sensorData[0], 1, -1, 0, 100));
  int new_y = int (map(sensorData[1], 1, -1, 0, 100));
  int new_z = int (map(sensorData[2], 1, -1, 0, 100));

  // constrainは、絶対に0〜100をはみ出さないようにする役割
  // 配列の数を超えないようにするため
  new_x = constrain(new_x, 0, 100);
  new_y = constrain(new_y, 0, 100);
  new_z = constrain(new_z, 0, 100);

  //10回の移動平均
  x = x*(99.0/100.0) + float(new_x)/100.0;
  y = y*(99.0/100.0) + float(new_y)/100.0;
  z = z*(99.0/100.0) + float(new_z)/100.0;
  //　---------- センサの値を整える ここまで ---------- //
